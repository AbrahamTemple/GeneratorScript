...
public class GenWorker extends GenBluePrint{
    ...
    void initGlobal() {
        ...
        String child = scanner("你的子项目名(没有就输入/)");
        if (!child.equals("/")) {
            instance.setChildPath("/"+child);
            instance.setPath(instance.getPathPrefix()+instance.getChildPath()+instance.getPathSuffix());
        }
    ...
    void initPackage() {
        ...
        instance.setParent(scanner("你当前项目的父目录(eg: com.example)"));
        instance.setModuleName(scanner("你的当前包名(eg: demo)"));
        pc.setModuleName(instance.getModuleName());
        pc.setParent(instance.getParent());
    ...
    void initStrategy() {
        ...
        strategy.setInclude(scanner("目标表名，多个用逗号隔开").split(","));
